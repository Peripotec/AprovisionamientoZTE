...
service tag gemport 1 ethuni eth_0/1 <span class="variable-highlight">${d.vlanInput}</span><br><br>
exit<br>
exit<br>`,
				copiarComando: (d) => `configure terminal\n
interface gpon-olt_1/${d.placa}/${d.puerto}\n
...
exit\n\n
pon-onu-mng gpon-onu_1/${d.placa}/${d.puerto}:${d.puertoLogico}\n
service tag gemport 1 ethuni eth_0/1 ${d.vlanInput}\n
exit\n
exit\n`,
			},
		],
...
exit\n`,
	...
	window.comandos = function () {
		...
		mostrarComandos(gen.render(config.comandos));
	}

	window.aprovisionamiento = function () {
		const gen = new CommandGenerator();
		mostrarComandos(gen.render(config.aprovisionamiento));
	}
